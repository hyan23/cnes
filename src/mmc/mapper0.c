...
void CNES_MAPPER_INIT_FUNC_NAME(0)(cnes_mmc_s* mmc)
{
    mmc->name = "NROM";
    mmc_sixteenKbank_t src = MMC_GET_16KPRG_BANK(mmc, 0);
    MMC_SWITCH_16K_BANK(mmc->layout->lower, src);
    src = MMC_GET_16KPRG_BANK(mmc, mmc->rom->header.prg_pgs - 1);
...
byte CNES_MAPPER_READ_FUNC_NAME(0)(cnes_mmc_s* mmc, word addr)
{
    return 0;
}

void CNES_MAPPER_WRITE_FUNC_NAME(0)(cnes_mmc_s* mmc, word addr, byte value)
{
    assert(FALSE);
}